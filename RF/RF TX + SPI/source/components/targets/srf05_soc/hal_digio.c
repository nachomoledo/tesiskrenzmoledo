...
#elif chip==2530 || chip==2531 || chip==2533 ||  chip==2541 || chip==2543 || chip==2545
...
#define PICTL_P0ICON_BM     0x01
#elif chip==1110 || chip==1111 || chip==2510 || chip==2511
#define IEN2_P1IE_BM        0x10
#define PICTL_P2IEN_BM      0x20
#define PICTL_P0IENH_BM     0x10
#define PICTL_P0IENL_BM     0x08
#define PICTL_P2ICON_BM     0x04
...
uint8 halDigioIntEnable(const digioConfig *p)
{
    ...
    case 0:
      P0IE = 1;
      // NOTE: it is not possible to enable/disable interrupts for one P0 pin input only
      if (p->pin <= 3) {
          PICTL |= PICTL_P0IENL_BM;    // enable interrupt on P0.{0-3}
      } else {
          PICTL |= PICTL_P0IENH_BM;    // enable interrupt on P0.{4-7}
      }
      break;
    case 1:
      IEN2 |= IEN2_P1IE_BM;
