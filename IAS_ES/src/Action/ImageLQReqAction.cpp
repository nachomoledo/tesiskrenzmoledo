...
ImageLQReqAction::ImageLQReqAction(TransmissionQueue *transmissionQueue, unsigned short id, QString imagePath)
    : m_transmissionQueue(transmissionQueue), m_imageId(id), m_imagePath(imagePath)
{
}

void ImageLQReqAction::execute()
{
...
//    /*TransmissionQueue::getInstance().addCommand(new GetImageRequestCommand(""));*/
    ...
    for (int i = 0; i < totalPackets; i++)
    {
        ...
        m_transmissionQueue->addCommand(new ImageLQCommandRes(m_imageId, i+1, totalPackets, data));
    }
